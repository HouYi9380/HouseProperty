...
//        return sb.toString();
    ...
    public static long evaluateExpression(String expression) {
        ...
        for (String token : tokens) {
            ...
            //这里如果是数字的话直接如数据的栈
                ...
                operandStack.push(Long.parseLong(token));   //将数字字符串转换成数字然后压入栈中
            }
        }
    ...
    private static void processAnOperator(Stack<Long> operandStack, Stack<Character> operatorStack) {
        char op = operatorStack.pop();  //弹出一个操作符
        long op1 = operandStack.pop();  //从存储数据的栈中弹出连个两个数用来和操作符op运算
